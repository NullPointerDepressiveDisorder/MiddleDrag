...
final class MouseEventGeneratorTests: XCTestCase {
    ...
    func testConcurrentStartDragDoesNotCrash() {
        ...
        guard let generator = self.generator else {
            XCTFail("Generator was not initialized")
            return
        }

        for i in 0..<10 {
            group.enter()
            concurrentQueue.async { [generator] in
                generator.startDrag(at: CGPoint(x: CGFloat(i * 10), y: CGFloat(i * 10)))
                group.leave()
            }
        }
    ...
    func testConcurrentStartAndCancelDoesNotCrash() {
        ...
        for i in 0..<20 {
            ...
            concurrentQueue.async { [generator] in
                if i % 2 == 0 {
                    generator.startDrag(at: CGPoint(x: CGFloat(i), y: CGFloat(i)))
                } else {
                    generator.cancelDrag()
                }
                group.leave()
